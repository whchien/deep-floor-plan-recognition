...
def convert_one_hot_to_image(one_hot,
                             dtype='float',
                             act=None):
    if act == 'softmax':
...
def decode_raw_infer(x):
    image = tf.io.decode_raw(x['image'], tf.uint8)
    return image


def preprocess_infer(img, size=512):
    img = tf.cast(img, dtype=tf.float32) / 255
    img = tf.reshape(img, [-1, size, size, 3])
    return img


def load_dataset(size=512, train=True):
    if train:
...
def load_images(directory, file_format='.jpg', size=512):
